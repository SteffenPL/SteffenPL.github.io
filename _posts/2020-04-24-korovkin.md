...
Continuity on an compact intervals gives us an uniform estimate
...
If we fix $$y$$ for a moment, we can apply $$T_n$$ to

$$
-b(ε, \vert \cdot - y \vert) ≤ f(⋅) - f(y) ≤ b(ε, \vert \cdot - y \vert)
$$

which gives due to positivity of the operators

$$
-T_n b(ε, \vert \cdot - y \vert) ≤ T_n f(⋅) - f(y) T_n x_0 ≤ T_n b(ε, \vert \cdot - y \vert).
$$

Now, we use that $$ b(ε, \vert x - y \vert) $$ is a polynomial of degree 2, which brings the assumption into play. This yields that the bounds converge to $$b(ε, \vert x - y \vert)$$ and
the term in the middle becomes $$(T_n f)(x) - f(y)$$, now the limit $$x → y$$ yields the claim. 
